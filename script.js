...
window.addEventListener("scroll", () => {
  if (mySkills.getBoundingClientRect().top < 300) {
    // or we can use if (ourSkills.scrollTop < 300)
    const progressSpans = document.querySelectorAll(
      " .about-stats .progress-bars .progress span"
    );
    progressSpans.forEach(
      (pSpan) => (pSpan.style.width = pSpan.dataset.progress)
    );
  }
});
